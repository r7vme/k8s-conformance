...
if [ -z "$FOCUS" ]; then
    ...
    GINKGO_PARALLEL=y go run hack/e2e.go -- --extract=v1.5.1 --v --test -check_version_skew=false \
    ...
    go run hack/e2e.go -- --extract=v1.5.1 --v --test -check_version_skew=false --test_args="--ginkgo.focus=\[Serial\].*\[Conformance\] --ginkgo.skip=$(skipped_test_names)"
else
    go run hack/e2e.go -- --extract=v1.5.1 --v --test -check_version_skew=false --test_args="--ginkgo.focus=$(escape_test_name "$FOCUS")"
fi
